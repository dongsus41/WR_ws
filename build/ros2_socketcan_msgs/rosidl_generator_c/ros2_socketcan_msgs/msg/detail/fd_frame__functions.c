  ...
  ros2_socketcan_msgs__msg__FdFrame__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ros2_socketcan_msgs__msg__FdFrame * data =
      (ros2_socketcan_msgs__msg__FdFrame *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ros2_socketcan_msgs__msg__FdFrame__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          ros2_socketcan_msgs__msg__FdFrame__fini(&data[i]);
        }
        free(data);
        return false;
      }
