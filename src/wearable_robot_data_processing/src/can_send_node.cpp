...
{
public:
    CANSendNode() : Node("can_send_node"), shutdown_(false)
    {
        // PWM 명령 구독
        pwm_subscription_ = this->create_subscription<wearable_robot_interfaces::msg::ActuatorCommand>(
            "actuator_command", 10,
            std::bind(&CANSendNode::command_callback, this, std::placeholders::_1));
        ...
        // CAN 통신용 스레드 시작
        can_thread_ = std::thread(&CANSendNode::can_thread_func, this);
    }
...
private:
    // 명령 수신 및 CAN 전송 콜백
    void command_callback(const wearable_robot_interfaces::msg::ActuatorCommand::SharedPtr msg)
    {
        // CAN 데이터 생성 (64바이트 크기)
        std::vector<uint8_t> data(64, 0);

        // PWM 값을 CAN 데이터에 복사 (6개 채널)
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<CANSendNode>());
    rclcpp::shutdown();
    return 0;
}
